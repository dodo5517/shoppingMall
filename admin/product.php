...
<?
	...
	$args="sel1=$sel1&sel2=$sel2&sel3=$sel3&sel4=$sel4&text1=$text1&page=$page";
...
<div class="row mx-1  justify-content-center">
	...
	<table class="table table-sm table-borderless m-0 p-0">
		<tr>
			<td width="20%" align="left" style="padding-top:8px">
				제품수 : <font color="red"><?=$total_count;?></font>
			</td>
			<td align="right">
	...
	<table class="table table-sm table-bordered table-hover mb-1">
		...
		?>
		<tr>
			<td><? echo $a_menu[$row["menu"]];?></td>
			<td><?=$row["code"];?></td>
			<td align="left"><?=$row["name"];?></td>
			<td align="right" class="px-2"><?=number_format($row["price"]);?></td>
			<td><? if($row["status"]==1) echo "판매중"; elseif($row["status"]==2) echo "판매중지"; elseif($row["status"]==3) echo "품절";?></td>
			<td><? if($row["icon_new"]==1) echo "New "; if($row["icon_hit"]==1) echo "Hit "; if($row["icon_sale"]==1) echo "Sale(".$row["discount"]."%".")";?></td>
			<td>

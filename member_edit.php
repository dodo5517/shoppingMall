...
<?
    ...
    $sql="select * from member where id= $cookie_id";     // id번째 자료 읽기
	...
	$birthday1=trim(substr($row["birthday"],0,4));        // 0번 위치에서 4자리 문자열 추출
    $birthday2=trim(substr($row["birthday"],5,2));        // 5번 위치에서 2자리
    $birthday3=trim(substr($row["birthday"],8,2));        // 8번 위치에서 2자리

<?
    ...
    $db= mysqli_connect("localhost", "shop33", "1234", "shop33"); //db
    if (!$db) exit ("DB연결에러");
	...
	$a_menu=array("메뉴선택","#MADE","OUTER","TOP","BLOUSE","SKIRT","BOTTOM","DRESS"); //메뉴
	$n_menu=count($a_menu);// 분류 개수

	$a_sort=array("정렬상태","신상품","인기상품","상품명","낮은가격","높은가격"); //정렬
	$n_sort=count($a_sort);
